...
export function ProductInfo({product}: Props) {
  ...
  function addToCart() {
    ...
    toast({
      ...
      action: (
        <Link href="/cart">
          <Button variant="link" className="flex items-center gap-x-2 whitespace-nowrap">
            <span>Open cart</span>
    ...
    <div className="mt-10 px-4 sm:mt-16 sm:px-0 lg:mt-0">
      ...
      <div className="mt-4">
        ...
        {product.sizes.map((size) => (
          <Button onClick={() => setSelectedSize(size)} key={size} variant={selectedSize === size?
          'default' : 'outline'} className="mr-2 mt-4">
            {getSizeName(size)}
          </Button>
        ))}
      </div>
